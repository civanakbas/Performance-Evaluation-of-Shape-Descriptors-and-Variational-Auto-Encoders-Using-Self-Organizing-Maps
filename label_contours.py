...
while(i < len(contour_list)):
    if i == 0:
        label_from_checkpoint = input(f"Currenty labeled objects: {len(labels)}, continue labeling from this point? True/False\n")
    ...
    plt.title(f"obj_{i}")
    plt.show()
    label = input("Please enter this images label [elongated, round, meander, crossed]: Type exit to stop labeling\n")
    if label == "exit":
        break
